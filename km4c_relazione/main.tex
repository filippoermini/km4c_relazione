...
\paragraph{<instanceIterationQuery>}
...
Proprietà:
\begin{itemize}
	\item \textt{type}: valore dell'uri \emph{rdf-syntax-ns#type} dell'istanza.
	\item \textt{name}: nome dell'istanza usato per referenziare l'istanza.
	\item \textt{isRoot}:se true indica se questa istanza contiene l'emento su cui inserire il valore generato dalla query.
	\item \textt{baseUri}: valore del \emph{baseURI} dell'istanza.
\end{itemize}
Elementi:
\begin{itemize}
	\item \textbf{properties} Elemento che contiene la lista di attributi che definiscono l'istanza.
\end{itemize}
\paragraph{properties}
Contiene la lista delle proprietà dell'istanza.
\begin{itemize}
	\item \textbf{list<prop>}
\end{itemize}
